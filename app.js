...
// real ya en base de datos (para mayor: negocio + tipo + nombre normalizado; para subcuenta:
// negocio + cuenta mayor + padre + nombre normalizado), un solo upsert con onConflict resuelve
// "existe o no" en una operación indivisible — dos llamadas concurrentes nunca pueden volver a
// crear un duplicado, sin importar qué tan rápido o al mismo tiempo se llamen. Esta función
// siempre crea subcuentas RAÍZ (sin padre); para subcuentas anidadas hay que crearlas aparte.
async function obtenerOCrearSubcuentaPorNombre(businessId, nombre, tipo) {
  ...
  const { data: sub, error: errSub } = await sb.from('fz_subcuentas')
    .upsert({ business_id: businessId, cuenta_mayor_id: mayor.id, nombre }, { onConflict: 'business_id,cuenta_mayor_id,padre_id_normalizado,nombre_normalizado' })
    .select('id').single();
...
async function eliminarFacturaProveedorConCascada(facturaId, businessId) {
  ...
  if (info.origen_poliza_id) {
    ...
    toast(
      `Esta factura nació de la Póliza de Diario #${poliza?.numero ?? ''} (${poliza?.fecha || ''}) — esa póliza puede tener ` +
      `otros movimientos además de esta provisión, así que para no afectarlos por accidente, elimínala desde Pólizas de Diario.`,
      'error'
    );
    return { ok: false };
  }
...
// Auxiliar de cuenta: todos los movimientos (Pólizas + Facturas de Proveedores desglosadas) de una
// o varias subcuentas juntas, en un rango de fechas, con Cargo/Abono separados y saldo corrido.
async function getMovimientosCuenta(businessId, subcuentaIds, startDate, endDate) {
  const calcularFilas = async (hasta) => {
    const { data: lineas } = await sb.from('fz_polizas_lineas').select('*').eq('business_id', businessId).eq('cuenta_tipo', 'subcuenta').in('subcuenta_id', subcuentaIds);
    const polizaIds = [...new Set((lineas||[]).map(l => l.poliza_id))];
    const { data: polizas } = polizaIds.length ? await sb.from('fz_polizas').select('id,fecha,numero,concepto').in('id', polizaIds) : { data: [] };
    const polizaMap = Object.fromEntries((polizas||[]).map(p => [p.id, p]));
    const filas = [];
    (lineas||[]).forEach(l => {
      const p = polizaMap[l.poliza_id];
      if (!p) return;
      if (startDate && p.fecha < startDate) return;
    ...
    (facturas||[]).forEach(f => {
      if (startDate && f.fecha < startDate) return;
    ...
    return filas.sort((a,b) => a.fecha.localeCompare(b.fecha) || 0);
  };
...
document.getElementById('closeMovimientosCuenta').addEventListener('click', () => {
  document.getElementById('modalMovimientosCuenta').classList.remove('show');
});
...
function filaArbolBalanceHtml(nodo, nivel, detalleAbiertoHtml) {
  ...
  let html = `<tr class="balance-subcuenta-row" data-subcuenta="${nodo.id}" data-nombre="${nodo.nombre.replace(/"/g,'&quot;')}" style="cursor:pointer;"><td style="padding-left:${indent}px;${estilo}">${nodo.nombre}</td><td class="num" style="${nivel === 1 ? 'font-weight:600;' : ''}">${fmtNeg(nodo.total)}</td></tr>`;
...
async function renderPL() {
  ...
  el.innerHTML = `
    <div class="pl-sticky-summary">
      ${plTagsHtml()}
      <p style="font-size:13px;color:var(--muted);margin:-4px 0 0;font-weight:600;">${estadoResultadosSubtitulo(periodo)}</p>
    </div>
    <div class="kpi-grid kpi-grid-compact" style="margin-bottom:14px;">
